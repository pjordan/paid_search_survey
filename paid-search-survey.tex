...
%Intuitively, we can think of the expected payment $\kappa_{i,j} p_i$ as the minimum expected bid (per impression) advertiser $i$ has to submit for the allocation algorithm not to change its decision.
...
%GSP maintains a similar property to VCG in that the amount the advertiser is charged is the minimum amount required to maintain its resulting slot (decision by the allocation algorithm).
...
%In contrast VCG (where solving the allocation problem can be combinatorial in general), GSP uses a greedy algorithm to rank the ads by $s_i$ first and allocates according to that ranking. 
%The contingent price charged a winning advertiser $i$ in GSP is the minimum amount advertiser $i$ has to pay in order to not change positions in the ranking.
%In loose analogy, we can think of GSP and VCG as being from the same family of paid search auctions where an advertiser pays the minimum amount necessary for the allocation algorithm not to change its decision.
%In this analogy, the GSP allocation algorithm considers a one-step look ahead (what is the minimum amount necessary to change my decision if I only consider the current slot), whereas VCG can be thought of as an infinite-step look ahead (what is the minimum amount necessary to not change my decision on the current slot given all remaining slots).
